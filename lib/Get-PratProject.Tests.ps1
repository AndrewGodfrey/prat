...
Describe "Get-PratProject" {
    ...
    It "Returns null when location is not inside any codebase root" {
        "@{ repos = @{ repo = @{ root = '$root/myrepo' } } }" | Out-File "TestDrive:\repoProfile.test.ps1"
        $result = &$scriptToTest -Location $root
        $result | Should -BeNull
    }

    It "Returns the repo directly when location doesn't match any subproject" {
        "@{ repos = @{ repo = @{ root = `$PSScriptRoot; subprojects = @{ sub = @{ path = 'lib/sub' } } } } }" | Out-File "TestDrive:\repoProfile.test.ps1"
        $result = &$scriptToTest -Location $root
    ...
    It "Picks the most-specific subproject when multiple match" {
        ...
        "@{ repos = @{ repo = @{ root = `$PSScriptRoot; subprojects = @{ sub = @{ path = 'lib/sub' }; nested = @{ path = 'lib/sub/nested' } } } } }" | Out-File "TestDrive:\repoProfile.test.ps1"
    ...
    It "Inherits codebase properties (e.g. cachedEnvDelta) into the sub-project" {
        New-Item -ItemType Directory "TestDrive:\lib\sub" -Force | Out-Null
        "@{ repos = @{ repo = @{ root = `$PSScriptRoot; cachedEnvDelta = 'env.ps1'; subprojects = @{ sub = @{ path = 'lib/sub' } } } } }" | Out-File "TestDrive:\repoProfile.test.ps1"
        $result = &$scriptToTest -Location (Get-Item "TestDrive:\lib\sub").FullName
    ...
    It "Sets workspace from subproject definition" {
        New-Item -ItemType Directory "TestDrive:\lib\sub" -Force | Out-Null
        "@{ repos = @{ repo = @{ root = `$PSScriptRoot; subprojects = @{ sub = @{ path = 'lib/sub'; workspace = 'mywsp' } } } } }" | Out-File "TestDrive:\repoProfile.test.ps1"
        $result = &$scriptToTest -Location (Get-Item "TestDrive:\lib\sub").FullName
